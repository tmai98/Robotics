...
void loop()
{
  ...
  delayMicroseconds(2);          // let it settle
  digitalWrite(r_trigPin,HIGH); // send high to front trigger device
  delayMicroseconds(10);         // let it settle
  digitalWrite(r_trigPin,LOW);
  fduration = pulseIn(f_echoPin, HIGH);
  fdistance = (rduration/2)/29.1;
 ...
 else if (fdistance > 110 && ldistance > 110){
  turnright();
  drivestraight();
 }
 else{
...
/*
 ...
 else if (fdistance > 110 && rdistance > 110){
  turnright();
  drivestraight();
 }
